# -*- coding: utf-8 -*-
"""
Plot experiment results for agent count experiments
"""
import os
import pandas as pd
import numpy as np
...
def plot_utility(data: dict, ax):
    """Plot objective value (utility) vs agent count"""
    ...
    ax.set_xlabel('Number of Agents', fontsize=22)
...
def plot_total_cost(data: dict, ax):
    """Plot total cost vs agent count"""
...
def plot_completion_ratio(data: dict, ax):
    """Plot task completion ratio vs agent count"""
    for algo_name, df in data.items():
        x = df['variable_value'].values
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Plot agent count experiment results')
