# -*- coding: utf-8 -*-
"""
Plot experiment results for failure rate experiments
"""
import os
import pandas as pd
import numpy as np
...
def plot_utility(data: dict, ax):
    ...
    ax.set_xlabel('Failure Rate', fontsize=22)
...
def plot_total_cost(data: dict, ax):
    """Plot total cost vs failure rate"""
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f'{v:.2f}' for v in all_x], fontsize=20)
...
def plot_completion_ratio(data: dict, ax):
    """Plot task completion ratio vs failure rate"""
    for algo_name, df in data.items():
        x = df['variable_value'].values
        y = df['expected_completion_rate'].values * 100
        yerr = df['expected_completion_rate_ci95'].values * 100
        ax.errorbar(x, y, yerr=yerr, label=algo_name,
                   color=COLORS.get(algo_name, 'gray'),
                   marker=MARKERS.get(algo_name, 'o'),
                   markersize=8, linewidth=2, capsize=3)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Plot failure rate experiment results')
