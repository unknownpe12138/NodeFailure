# -*- coding: utf-8 -*-
"""
Plot experiment results for task count experiments
"""
import os
import pandas as pd
import numpy as np
...
def plot_utility(data: dict, ax):
    """Plot objective value (utility) vs task count"""
...
def plot_completion_ratio(data: dict, ax):
    """Plot task completion ratio vs task count"""
    for algo_name, df in data.items():
        x = df['variable_value'].values
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Plot task count experiment results')
