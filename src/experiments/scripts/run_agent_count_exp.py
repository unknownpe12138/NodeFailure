...
def run_agent_count_experiment(num_runs: int = NUM_RUNS, verbose: bool = True):
    """
    运行智能体数量变量实验（多算法对比）
    ...
    """
    print("="*60)
    print("智能体数量变量实验（多算法对比）")
    print("="*60)
    print(f"变量范围: {VARIABLE_RANGES['num_agents']}")
    ...
    results = runner.run_variable_experiment(
        ...
        variable_values=VARIABLE_RANGES['num_agents'],
        base_params=DEFAULT_PARAMS
    )

    # 打印结果
    runner.metrics_collector.print_results('num_agents', results)

    # 导出CSV
    filepath = runner.metrics_collector.export_variable_results('num_agents', results)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='智能体数量变量实验（多算法对比）')
